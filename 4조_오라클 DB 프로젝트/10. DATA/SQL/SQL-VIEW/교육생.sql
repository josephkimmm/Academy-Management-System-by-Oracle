                    ...
                    on o.open_curriculum_id = oc.open_curriculum_id
                        inner join mroom r
                            on oc.room_id = r.room_id
                                where s.student_id = 285;
                            
select * from vwStudentInfo;
drop view vwStudentInfo;

-- 과목별 성적 정보 출력
create or replace view vwStudentResult 
as
select
...
create or replace view vwStudentAttendance
as
select
    s.student_id as 교육생번호,
    s.name as 교육생이름,
