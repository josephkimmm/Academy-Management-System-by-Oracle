...
--        INNER JOIN vDistribution d
--            ON b.개설과정중간번호 = d.개설과정중간번호
--                INNER JOIN vScroeAddCheck sac
--                    ON d.과정명 = b.과정명;
    
--select * from vSubAndCurriAndBook;
--특정 과정 출결
--CREATE OR REPLACE VIEW vParticularAttendance
--AS
--SELECT c.title 과정명, 
--        st.name 교육생이름, 
--        t.clock_in 날짜, 
...
--        INNER JOIN mOpenCurriculum oc
--            ON c.curriculum_id = oc.curriculum_id
--                INNER JOIN mOpenCurriStudent ocs
--                    ON ocs.open_curriculum_id = oc.open_curriculum_id
--                        INNER JOIN mStudent st
--                            ON ocs.student_id = st.student_id
--                                INNER JOIN mTimeKeeping t
--                                    ON st.student_id = t.student_id
--                                        INNER JOIN mAttendanceType a
--                                            ON t.attendance_Type_id = a.attendance_Type_id; 
--      
--select * from vParticularAttendance;
